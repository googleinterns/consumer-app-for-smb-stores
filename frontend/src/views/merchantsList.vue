  ...
  created() {
    ...
    mdb.on("child_added", snapshot => {
      ...
      rdb.once("value").then(snapshot => {
        var merchantRef = snapshot.val();
        var Quality = merchantRef.TotalRatings.Quality;
        var Timeliness = merchantRef.TotalRatings.Timeliness;
        var ordersNum = merchantRef.Number_of_Orders.count;
